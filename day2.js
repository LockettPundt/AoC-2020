...
// part one.
const passwordValidator = (arr) => {
  return arr.reduce((sum, x) => {
    const [cypher, password] = x.split(': ')
    const [range, letter] = cypher.split(' ')
    const [min, max] = range.split('-')
    const numLetters = password.split('').filter(x => x === letter).length
    if (min <= numLetters && numLetters <= max) {
      sum++
    }
    return sum
  }, 0)
...
// part two
const passwordValidatorTwo = (arr) => {
  return arr.reduce((sum, x) => {
    const [cypher, password] = x.split(': ')
    const [range, letter] = cypher.split(' ')
    const [min, max] = range.split('-')
    const firstLetter = password[min - 1] === letter && password[max - 1] !== letter
    const secondLetter = password[min - 1] !== letter && password[max - 1] === letter
...
console.log(passwordValidator(inputArray)) // 536
console.log(passwordValidatorTwo(inputArray)) // 558